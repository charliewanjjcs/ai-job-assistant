...
| 模块 | 路径 | 状态 | 锁定 |
|------|------|------|------|
| 入口/表单/展示 | `app/main.py` | locked(演进)：薪资改时薪/月薪/年薪+币种选择；新增语言(语言+3档熟练度)、到岗时间(手动)；目标岗位→理想工作(手动)；JD 自动识别语言/到岗偏好且**粘贴 JD 原文自动回填「必需/加分技能」输入框**；新增「语言匹配」「到岗匹配」标签页；预期薪资左(计薪方式)/中(纯数字)/右(币种)三栏，修复 KeyError；性格框提示移入 placeholder 并去掉"取简历原话"；语言区删冗余"已掌握语言"副标题；必需技能原名"必选技能" | core-refine-locked |
