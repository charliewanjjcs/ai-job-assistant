...
| 存储层 | `app/storage.py` | SQLite（`users`/`profiles`/`skill_library`/`verification_codes`/`analysis_results`）：`init_db`/`set_db_path`/`get_db_path`/`get_user`/`get_user_by_provider`/`get_user_by_email`/`get_or_create_user`/`create_email_user`/`authenticate_email`/`load_profile`/`save_profile`/`has_profile_data`/`list_skills`/`add_skill`/`remove_skill`/`is_custom_skill`/`save_verification_code`/`verify_code`/`save_analysis_result`/`list_analysis_results`/`get_analysis_result`/`delete_analysis_result`/`clear_analysis_results`/`deserialize_report`（`analysis_results` 冗余 role/company/generated_at/skill_score/salary_verdict 供列表，`report_json` 存 `report.model_dump(mode="json")`） | active | — |
| 登录态 | `app/auth.py` | `current_user_id`/`current_display`/`is_logged_in`/`require_login`/`logout`/`login_provider`/`login_email`/`register_email`/`send_phone_code`/`login_phone`/`persist_login`/`try_restore_login`（本地模拟账户：微信/QQ/谷歌/手机号/邮箱；邮箱密码哈希；验证码存 DB+TTL；`data/session.json` 保持登录；logout 加 `_logged_out` 会话标志+删除失败清空文件） | active | — |
