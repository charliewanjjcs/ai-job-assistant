...
| 登录表单 | `app/components/login_form.py` | `render_login_form(prefix)`（微信/QQ（模拟）/手机/邮箱/谷歌（真实 OAuth，见 `app/google_oauth.py`），prefix 区分挂载点 key），供 sidebar popover 与首页 dialog 复用 | active | — |
| 谷歌 OAuth | `app/google_oauth.py` | `is_configured`/`decode_id_token`（解析 Google id_token 的 sub/email/name）/`upsert_google_user`（按邮箱合并已有账户或按 google+sub 创建，落到现有 users 表）/`render_login_button(key_prefix)`（streamlit_oauth.OAuth2Component popup 流程；**组件 key 必须 = f"{key_prefix}_oauth" 随挂载点变化**，否则侧栏 popover 与首页 dialog 同渲染会触发 StreamlitDuplicateElementKey；成功后写 session_state 并 rerun；仅登录时一次网络/落库） | active | — |
| 数据层纯函数 | `app/state.py` | `build_profile() -> UserProfile`、`build_jd() -> JdInfo`、`on_jd_text_change()`、`DemoLLM` + 选项常量 | active | — |
| 存储层 | `app/storage.py` | 可切换后端：设 `DATABASE_URL`(postgresql://) 且 psycopg2 可用 → Postgres（DDL 用 SERIAL+CURRENT_TIMESTAMP，INSERT 走 RETURNING id），否则退回本地 SQLite（`data/app.db` 或 `APP_DB_PATH`、DDL 保持历史原样零回归）；占位符 `?→%s` 自动适配。Postgres 连接经 `_PgConn` 包装以暴露与 sqlite3 一致的 `.execute()`/`.commit()`/`.close()` 接口（**psycopg2 原始连接无 .execute()，否则 init_db/查询会 AttributeError**）。函数同：`init_db`（**已加同进程记忆化：按「后端+路径」只执行一次 DDL，避免每次 rerun 跑 11 条建表/建索引语句造成全局 ~2 秒卡顿**）/`set_db_path`/`get_db_path`/`is_postgres`/`get_user`/`get_user_by_provider`/`get_user_by_email`/`get_or_create_user`/`create_email_user`/`authenticate_email`/`load_profile`/`save_profile`/`has_profile_data`/`list_skills`/`add_skill`（按 `skill_dedupe_key` 去重）/`remove_skill`/`is_custom_skill`/`save_verification_code`/`verify_code`/`save_analysis_result`/`list_analysis_results`/`get_analysis_result`/`get_analysis_report`（详情轻量查询，**不取 jd_text/skill_score/salary_verdict**，减小跨 Neon 传输体积）/`delete_analysis_result`/`clear_analysis_results`/`deserialize_report` | active | — |
...
| 技能去重归一化 | `app/skill_norm.py` | `skill_dedupe_key(skill)`（小写、去空格/连字符等非字母数字、去末尾复数 s；用于「添加技能」候选去重/已添加去重，**不合并语义不同技能**，与 core 的 SKILL_SYNONYMS 匹配相互独立） | active | — |
| 技能编辑器 | `app/components/skill_editor.py` | `suggest_skills(query, vocab, existing, limit)`（候选与已添加均按 `skill_dedupe_key` 去重：detail-oriented/detail oriented、attention to detail/attention to details 视为同一）+ `skill_editor(user_id)`（Workday 式联想→点选→标签可删→回车自定义，复用 `core.parsers` 词库） | active | — |
...
| 个人资料页 | `app/views/profile.py` | `render()`（DB→session_state、技能编辑器、语言、薪资、性格、保存；上传 PDF 写 `tempfile.mkstemp` 唯一临时文件，**解析出的技能写入 DB 后同步 `session_state["skills"]` 缓存**，否则技能编辑器不显示） | active | — |
