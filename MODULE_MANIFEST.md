...
| 谷歌 OAuth | `app/google_oauth.py` | `is_configured`/`decode_id_token`（解析 Google id_token 的 sub/email/name）/`upsert_google_user`（按邮箱合并已有账户或按 google+sub 创建，落到现有 users 表）/`render_login_button`（streamlit_oauth.OAuth2Component popup 流程，成功后写 session_state 并 rerun；仅登录时一次网络/落库，不增加日常操作开销） | active | — |
