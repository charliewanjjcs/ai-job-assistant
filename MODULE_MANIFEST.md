...
| 文本占位解析 | `core/parsers.py` | `parse_resume_text(...)`, `parse_jd_text(...)`, `extract_skills/extract_personality/parse_expected_salary/extract_jd_languages/extract_prefers_immediate`（技能=词表(技术/办公/分析工具)+软技能词表，逐词边界匹配，绝不把"技能"标签后整段文字当技能；性格=显式性格标签字面 或 从「个人总结/自我评价」段落抽取性格词；薪资识别时薪/月薪/年薪+币种；JD 抽语言/到岗偏好） | locked | core-refine-locked |
...
| 模块 | 路径 | 状态 | 锁定 |
|------|------|------|------|
| 入口/表单/展示 | `app/main.py` | locked(演进)：薪资改时薪/月薪/年薪+币种选择；新增语言(语言+3档熟练度)、到岗时间(手动)；目标岗位→理想工作(手动)；JD 自动识别语言/到岗偏好；新增「语言匹配」「到岗匹配」标签页；预期薪资改为左(计薪方式)/中(纯数字)/右(币种)三栏布局，修复 selectbox 元组导致的 KeyError | core-refine-locked |
